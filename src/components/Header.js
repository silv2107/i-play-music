...
function Header({ children, theColor, brightness, symbolColor, theBackground, theModifier }) {
	return (
		<article className="header" style={{ backgroundColor: `${theBackground}` }}>
			...
			<h4 className="header__title" style={{ color: theColor }}>
				{children}
			</h4>
			<div style={theModifier} className="header__search">
				<img style={{ filter: brightness }} src="/icons/search.svg" alt="search icon" />
			</div>
		</article>
	);
}
